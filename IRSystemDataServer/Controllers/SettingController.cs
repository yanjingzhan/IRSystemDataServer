...
namespace IRSystemDataServer.Controllers
{
    ...
    {
        [HttpPost]
        public async Task<IHttpActionResult> Add([FromBody]Setting entity)
        {
            DataContext.Setting.Add(entity);
            await DataContext.SaveChangesAsync();

            return Json(new ResponseData<Setting>() { Data = entity });
        }

        [HttpPost]
        public async Task<IHttpActionResult> Update(long id, [FromBody]Setting entity)
        {
            var resp = new ResponseData<Setting>();
            try
            {
                var query = from a in DataContext.Setting
                            where a.Id == id
                            select a;
                var error = "";
            ...
            catch (Exception ex)
            {
                ...
                Logger.LogError("Update Role Setting error", ex);
            }
            return Json(resp);
        }
        ...
        public IHttpActionResult Query([FromUri]string name)
        {
            var query = from a in DataContext.Setting
                        ...
                        select a;
            var responseData = new ResponseData<IEnumerable<Setting>>()
            {
                Message = "",
                Data = query
            };
